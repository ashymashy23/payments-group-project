...
class Payment extends Component {
  constructor() {
    ...
    // const currency = this.state.selectedCurrency;
		fetch("https://exchangeratesapi.io/api/latest?base=" + this.state.selectedCurrency)
      .then(data => data.json())
      .then(response => {
	...
	render() {
		return <div className="CalcPayment">
        ...
        <div className="CalcPayment-control">
          ...
          <input onChange={this.handleChange} className="CalcPayment-amount" type="text" defaultValue="0.00" />
          is worth <span className="CalcPayment-result">
            {this.state.amount}
          </span> in GBP.
